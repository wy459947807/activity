...
class WuweiArticleModel extends CommonModel{
    //自动验证
    ...
    public function dataList($params){
        $this->sqlFrom = " cmf_wuwei_article ";              //数据库查询表
        ...
        if (!empty($params['keyword'])) {
            $this->sqlWhere .= " and  (title like '%s') ";
    ...
    public function dataUpdate($params) {
        ...
        try {
            if(!empty($params['id'])){
                $model->table(C('DB_PREFIX') . 'wuwei_article')->where(array("id" =>$params['id']))->save($params);
            }else{ 
                $params['create_time']=date("Y-m-d H:i:s");
                $model->table(C('DB_PREFIX') . 'wuwei_article')->add($params);
            }
