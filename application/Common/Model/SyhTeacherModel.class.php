...
class SyhTeacherModel extends CommonModel{
    //自动验证
    ...
    public function dataList($params){
        $this->sqlFrom = " cmf_syh_teacher ";              //数据库查询表
    ...
    public function dataUpdate($params) {
        ...
        try {
            if(!empty($params['id'])){
                $model->table(C('DB_PREFIX') . 'syh_teacher')->where(array("id" =>$params['id']))->save($params);
            }else{ 
                $params['create_time']=date("Y-m-d H:i:s");
                $model->table(C('DB_PREFIX') . 'syh_teacher')->add($params);
            }
