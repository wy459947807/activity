...
class RuidaAppointModel extends CommonModel{
    //自动验证
    ...
    public function dataList($params){
        $this->sqlFrom = " cmf_ruida_appoint ";              //数据库查询表
    ...
    public function dataUpdate($params) {
        ...
        try {
            if(!empty($params['id'])){
                $model->table(C('DB_PREFIX') . 'ruida_appoint')->where(array("id" =>$params['id']))->save($params);
            }else{ 
                $params['create_time']= time();
                $model->table(C('DB_PREFIX') . 'ruida_appoint')->add($params);
            }
