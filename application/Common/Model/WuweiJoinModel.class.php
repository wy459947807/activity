...
class WuweiJoinModel extends CommonModel{
    //自动验证
    ...
    public function dataList($params){
        $this->sqlFrom = " cmf_wuwei_join ";              //数据库查询表
    ...
    public function dataUpdate($params) {
        ...
        try {
            if(!empty($params['id'])){
                $model->table(C('DB_PREFIX') . 'wuwei_join')->where(array("id" =>$params['id']))->save($params);
            }else{ 
                $params['create_time']=date("Y-m-d H:i:s");
                $model->table(C('DB_PREFIX') . 'wuwei_join')->add($params);
            }
