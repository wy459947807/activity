...
class TianyiMonthlyController extends AdminbaseController{
	...
	public function index(){
            ...
            $params['pageLimit'] = 20;
            $dataList = $this->TianyiMonthly->dataList($params);
        ...
        public function update() {
            if (IS_POST) {
                ...
                $retInfo = $this->TianyiMonthly->dataUpdate($params);
                $this->ajaxReturn($retInfo);
            } else {
                $id = I('get.id', 0, 'intval');
                $dataInfo = $this->TianyiMonthly->where(array("id" => $id))->find();
        ...
        public function  deleteInfo(){
            if(IS_POST){
                $ids = I('post.ids/a');
                if(!empty($ids)){
                    if ($this->TianyiMonthly->where(array('id' => array('in', $ids)))->delete() !== false) {
        ...
        public function updateStatus() {
            if(IS_POST){
                ...
                if(!empty($ids)){
                    if ($this->TianyiMonthly->where(array('id' => array('in', $ids)))->save(array('status' => $status)) !== false) {
                        $this->ajaxReturn(array( "status" => 200, "msg" => "操作成功！","data" => ""));
                    } else {
                        $this->ajaxReturn(array( "status" => 500, "msg" => "操作失败！","data" => ""));
                    }
