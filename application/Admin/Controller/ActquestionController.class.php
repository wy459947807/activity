...
class ActquestionController extends AdminbaseController{
	...
	public function index(){
            ...
            $params['pageLimit'] = 20;
            $dataList = $this->question_model->questionList($params);
        ...
        public function update() {
            if (IS_POST) {
                ...
                $retInfo = $this->question_model->dataUpdate($params);
                $this->ajaxReturn($retInfo);
            } else {
                $id = I('get.id', 0, 'intval');
                $dataInfo = $this->question_model->where(array("id" => $id))->find();
        ...
        public function updateStatus() {
            if(IS_POST){
                ...
                if(!empty($ids)){
                    if ($this->question_model->where(array('id' => array('in', $ids)))->save(array('status' => $status)) !== false) {
