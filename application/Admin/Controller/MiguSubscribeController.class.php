...
class MiguSubscribeController extends AdminbaseController{
	...
	public function index(){
            ...
            $params['pageLimit'] = 20;
            $dataList = $this->MiguSubscribe->dataList($params);
        ...
        public function update() {
            if (IS_POST) {
                ...
                $retInfo = $this->MiguSubscribe->dataUpdate($params);
                $this->ajaxReturn($retInfo);
            } else {
                $id = I('get.id', 0, 'intval');
                $dataInfo = $this->MiguSubscribe->where(array("id" => $id))->find();
        ...
        public function  deleteInfo(){
            if(IS_POST){
                $ids = I('post.ids/a');
                if(!empty($ids)){
                    if ($this->MiguSubscribe->where(array('id' => array('in', $ids)))->delete() !== false) {
        ...
        public function updateStatus() {
            if(IS_POST){
                ...
                if(!empty($ids)){
                    if ($this->MiguSubscribe->where(array('id' => array('in', $ids)))->save(array('status' => $status)) !== false) {
                        $this->ajaxReturn(array( "status" => 200, "msg" => "操作成功！","data" => ""));
                    } else {
                        $this->ajaxReturn(array( "status" => 500, "msg" => "操作失败！","data" => ""));
                    }
