...
<body>
    <?php $statusArray=array(1=>"已订阅",2=>"已退订"); $typeArray=array(1=>"线上优惠券",2=>"线下优惠券"); ?>
    <div class="wrap js-check-wrap">
        <ul class="nav nav-tabs">
            <li class="active"><a href="<?php echo U('MiguSubscribe/index');?>">订阅列表</a></li>
        </ul>
        <form class="well form-search" method="post" action="<?php echo U('MiguSubscribe/index');?>"> 
            ...
            <a class="btn btn-danger" href="<?php echo U('MiguSubscribe/index');?>">清空</a>
        </form>
        ...
        <table class="table table-hover table-bordered">
            ...
            <?php if(is_array($dataList)): foreach($dataList as $key=>$vo): ?><tr>
                    ...
                    <td>  
                    <?php if($vo["unsub_time"] != 0): echo date("Y-m-d H:i:s",$vo['unsub_time']);?>
                    <?php else: ?>
                        未退订<?php endif; ?>
                    </td>
                    <td><?php echo ($statusArray[$vo['status']]); ?></td>
                    <td>
                        <a href="<?php echo U('MiguTicketSend/index',array('mobile'=>$vo['mobile']));?>">优惠券详情</a> 
                    </td>
    ...
    <script>
        function updateStatus(id,status){
            layer.confirm('你确定更新状态吗？', function(){
                getRemoteData({ids:[id],status:status},"<?php echo U('MiguSubscribe/updateStatus');?>",1);
            });
        }
        
        function deleteInfo(id){
            layer.confirm('你确定删除吗？', function(){
                getRemoteData({ids:[id]},"<?php echo U('MiguSubscribe/deleteInfo');?>",1);
            });
        }
    </script>
</body>
