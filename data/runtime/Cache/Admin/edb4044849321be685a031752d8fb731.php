...
<body>
    ...
    <div class="wrap js-check-wrap">
        <ul class="nav nav-tabs">
            <li class="active"><a href="<?php echo U('DykUsers/index');?>">人员列表</a></li>
          
        </ul>
        <form class="well form-search" method="post" action="<?php echo U('DykUsers/index');?>"> 
            ...
            <a class="btn btn-danger" href="<?php echo U('DykUsers/index');?>">清空</a>
        </form>
        ...
        <table class="table table-hover table-bordered">
            ...
            <?php if(is_array($dataList)): foreach($dataList as $key=>$vo): ?><tr>
                    ...
                    <td>
                        <?php if($vo['card'] != null): ?><a href="javascript:parent.image_preview_dialog('<?php echo ($vo["card"]); ?>');" style=" margin-right: 10px"><img  width="30" height="30" src="<?php echo ($vo["card"]); ?>"/></a><?php endif; ?>
                    </td>
                    <td><?php echo ($vo["name"]); ?></td>
                    <td><?php echo ($vo["country"]); ?></td>
                    <td><?php echo ($vo["province"]); ?></td>
                    <td><?php echo ($vo["city"]); ?></td>
                    <td><?php echo ($vo["nature"]); ?></td>
                    <td><?php echo ($statusArray[$vo['status']]); ?></td>
                    <td><?php echo ($vo["create_time"]); ?></td>
                    <td>
                        <a href="<?php echo U('DykUsers/detail',array('id'=>$vo['id']));?>">详情</a> | 
        ...
        <div class="table-actions">
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('DykUsers/updateStatus');?>" data-subcheck="true"  data-msg="你确定停用吗？">批量禁用</button>
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('DykUsers/deleteInfo');?>" data-subcheck="true"  data-msg="你确定删除吗？">批量删除</button>
        </div>
    ...
    <script>
        function updateStatus(id,status){
            layer.confirm('你确定更新状态吗？', function(){
                getRemoteData({ids:[id],status:status},"<?php echo U('DykUsers/updateStatus');?>",1);
            });
        }
        
        function deleteInfo(id){
            layer.confirm('你确定删除吗？', function(){
                getRemoteData({ids:[id]},"<?php echo U('DykUsers/deleteInfo');?>",1);
            });
        }
    </script>
</body>
