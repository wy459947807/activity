    ...
    <div class="wrap js-check-wrap">
        ...
        <form class="well form-search" method="post" action="<?php echo U('MiguTicketSend/index',array('mobile'=>$formget['mobile']));?>"> 
            ...
            <select name="status">
                ...
                <?php if(is_array($statusArray)): foreach($statusArray as $key=>$vo): $selected=$formget['status']==$key?"selected":""; ?>
        ...
        <table class="table table-hover table-bordered">
            ...
            <?php if(is_array($dataList)): foreach($dataList as $key=>$vo): ?><tr>
                    ...
                    <td>
                        ...
                        <?php if($vo["status"] == 1): ?><a href="javascript:void(0)" onclick="updateStatus(<?php echo ($vo["id"]); ?>,2)">已使用</a> |   
                        <?php else: ?>
                            <a href="javascript:void(0)" onclick="updateStatus(<?php echo ($vo["id"]); ?>,1)">未使用</a> |<?php endif; ?>
        ...
        <div class="table-actions">
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('MiguTicketSend/updateStatus');?>" data-subcheck="true"  data-msg="你确定设置已使用吗？">批量使用</button>
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('MiguTicketSend/deleteInfo');?>" data-subcheck="true"  data-msg="你确定删除吗？">批量删除</button>
        </div>
    ...
    <script>
        function updateStatus(id,status){
            layer.confirm('你确定更新状态吗？', function(){
                getRemoteData({ids:[id],status:status},"<?php echo U('MiguTicketSend/updateStatus');?>",1);
            });
        }
        
        function deleteInfo(id){
            layer.confirm('你确定删除吗？', function(){
                getRemoteData({ids:[id]},"<?php echo U('MiguTicketSend/deleteInfo');?>",1);
            });
        }
    </script>
</body>
