...
<body>
    ...
    <div class="wrap js-check-wrap">
        <ul class="nav nav-tabs">
            <li class="active"><a href="<?php echo U('SyhCourse/index');?>">课程列表</a></li>
            <li><a href="<?php echo U('SyhCourse/update');?>">添加课程</a></li>
        </ul>
        <form class="well form-search" method="post" action="<?php echo U('SyhCourse/index');?>"> 
            ...
            <a class="btn btn-danger" href="<?php echo U('SyhCourse/index');?>">清空</a>
        </form>
        ...
        <table class="table table-hover table-bordered">
            ...
            <?php if(is_array($dataList)): foreach($dataList as $key=>$vo): ?><tr>
                    ...
                    <td><?php echo ($vo["charge"]); ?>/人</td> 
                    <td><?php echo ($statusArray[$vo['status']]); ?></td>
                    <td><?php echo ($vo["create_time"]); ?></td>
                    <td>
                        <a href="<?php echo U('SyhCourse/update',array('id'=>$vo['id']));?>">修改</a> | 
        ...
        <div class="table-actions">
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('SyhCourse/updateStatus');?>" data-subcheck="true"  data-msg="你确定停用吗？">批量禁用</button>
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('SyhCourse/deleteInfo');?>" data-subcheck="true"  data-msg="你确定删除吗？">批量删除</button>
        </div>
    ...
    <script>
        function updateStatus(id,status){
            layer.confirm('你确定更新状态吗？', function(){
                getRemoteData({ids:[id],status:status},"<?php echo U('SyhCourse/updateStatus');?>",1);
            });
        }
        
        function deleteInfo(id){
            layer.confirm('你确定删除吗？', function(){
                getRemoteData({ids:[id]},"<?php echo U('SyhCourse/deleteInfo');?>",1);
            });
        }
    </script>
</body>
