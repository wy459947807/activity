...
<body>
    ...
    <div class="wrap js-check-wrap">
        <ul class="nav nav-tabs">
            <li class="active"><a href="<?php echo U('TianyiOrder/index');?>">订单列表</a></li>
        </ul>
        <form class="well form-search" method="post" action="<?php echo U('TianyiOrder/index');?>"> 
            ...
            <a class="btn btn-danger" href="<?php echo U('TianyiOrder/index');?>">清空</a>
        </form>
        ...
        <table class="table table-hover table-bordered">
            ...
            <?php if(is_array($dataList)): foreach($dataList as $key=>$vo): ?><tr>
                    ...
                    <td>
                        <a href="<?php echo U('TianyiOrder/detail',array('id'=>$vo['id']));?>">详情</a> | 
                        <?php if($vo["status"] == 1): ?><a href="javascript:void(0)" onclick="updateStatus(<?php echo ($vo["id"]); ?>,2)">已支付</a> |   
                        <?php else: ?>
                            <a href="javascript:void(0)" onclick="updateStatus(<?php echo ($vo["id"]); ?>,1)">未支付</a> |<?php endif; ?>
        ...
        <div class="table-actions">
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('TianyiOrder/updateStatus');?>" data-subcheck="true"  data-msg="你确定停用吗？">批量支付</button>
            <button class="btn btn-primary btn-small js-ajax-submit"  type="submit" data-action="<?php echo U('TianyiOrder/deleteInfo');?>" data-subcheck="true"  data-msg="你确定删除吗？">批量删除</button>
        </div>
    ...
    <script>
        ...
        function deleteInfo(id){
            layer.confirm('你确定删除吗？', function(){
                getRemoteData({ids:[id]},"<?php echo U('TianyiOrder/deleteInfo');?>",1);
            });
        }
    </script>
</body>
